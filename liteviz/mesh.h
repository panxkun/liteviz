...
public:
    ...
    void draw(const std::shared_ptr<Shader>& shader, const Viewport& viewport) {
        ...
        shader->bind();
        shader->set_uniform("ProjMat", transform);
        shader->set_uniform("Alpha", 0.5f);
        shader->set_attribute("Color", plane_colors);
        shader->set_attribute("Position", plane_positions);
        shader->set_indices(plane_indices);
        shader->draw_indexed(GL_TRIANGLES, 0, plane_indices.size());
        shader->unbind();
